...
import { useGetCategoriSlugQuery, useGetSaborAllQuery } from "../../graphql/generated"
import { ComponentMenu } from "./ComponentMenu"

export function GetCategoriSlug() {
  const { slug } = useParams<{ slug: string }>()

  const { data } = useGetCategoriSlugQuery({
    variables: { slug: slug },
  })
  console.log(data)
    ...
    <div className="w-full bg-black-900 ">
      <div className="max-w-5xl  mx-auto py-4 px-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2  gap-7">
          {data?.categorias.map(sabor => {
            return (
              <>{sabor.sabors.map(sabor => {
                return <ItemMenu
                  nome={sabor.nome}
                  valor={sabor.valor}
                  foto={sabor.foto?.url}
